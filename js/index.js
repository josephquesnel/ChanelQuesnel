...
$(window).on('animationend',function(){
  ...
  loadertext.addEventListener('animationend',function(){
    ...
    gsap.to(loadertext,.5,{
      opacity:0,
      onComplete:function(){
        gsap.to(loadertext,0,{
          display:'none'
        })
        gsap.to('#loader',.3,{   //LOADING PAGE TRANSITION
          y:'-100%',
          ease:'Expo.easeInOut'
      })
      }
...
$(function(){
 $('.menu-bar').on('click',function(){ //WHEN MENUBAR IS CLICKED BRING NAVIGATION UP
    gsap.to('#navigation',.2,{
    y:'0%',
    ease:'Expo.easeInOut',
    onComplete:function(){ //WHEN NAVIGATION ANIMATION IS COMPLETED DO THE FOLLOWING
      gsap.to('.navigation-opacity',.3,{  //GET ELEMENTS OF CLASS 'NAVIGATION-OPACITY' AND TURN THEIR OPACITY TO 1
        opacity:1,
        stagger:.1
      })
    }
  ...
  $('.navigation-close').on('click',function(){ //WHEN NAVIGATION CLOSE IS CLICKED ANIMATE NAVIGATION DOWN
    ...
    gsap.to('.navigation-opacity',.2,{  //GET ELEMENTS OF CLASS 'NAVIGATION-OPACITY' AND TURN THEIR OPACITY TO 0
    opacity:0,
    stagger:.05,
    onComplete:function(){ //WHEN OPACITY ANIMATION IS COMPLETED DO THE FOLLOWING
      gsap.to('#navigation',.3,{
        y:'100%',
        ease:'Expo.easeInOut',
    })
  }
  ...
  links.forEach(link => link.addEventListener('click',function(){ //on click on page link element
    ...
    if(document.querySelector(page)){
      ...
      function displaybreaker(){
          ...
          gsap.to('.navigation-opacity',.2,{  //close navigation
          opacity:0,
          stagger:-.05,
          onComplete:function(){ 
            gsap.to('#navigation',.3,{
